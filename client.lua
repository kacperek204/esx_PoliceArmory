	...
	for k,v in pairs(Config.WeaponsInArmory) do
		...
		for z,x in pairs(weapons) do
			if x == v.weaponHash then
				takenOut = true
				table.insert(elements,{label = v.label .. " --- "..('<span style="color:red;">%s</span>'):format("Wyjęte"), weaponhash = v.weaponHash, lendable = false})
			end
		end
		if takenOut == false then
			table.insert(elements,{label = v.label .. " --- "..('<span style="color:green;">%s</span>'):format("W magazynie"), weaponhash = v.weaponHash, lendable = true})
		end
	end
...
function ListOfAttachments(type,name,weaponhash,attachment,flashlight,scope,suppressor)
	...
	if flashlight then
		...
		if state then
			text = "Flashlight: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Flashlight: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
		end
	...
	if scope then
		...
		if state then
			text = "Scope: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Scope: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
		end
	...
	if suppressor then
		...
		if state then
			text = "Suppressor: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Suppressor: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'policeArmory_list_of_attachments', {
		...
		if data.current.value == flashlight then
			if data.current.state then
				newData.label = "Flashlight: "..('<span style="color:green;">%s</span>'):format("Założone")
				...
				newData.label = "Flashlight: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
		...
		elseif data.current.value == scope then
			if data.current.state then
				newData.label = "Scope: "..('<span style="color:green;">%s</span>'):format("Założone")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeEquipped,name))
			else
				newData.label = "Scope: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeRemove,name))
			end
		elseif data.current.value == suppressor then
			if data.current.state then
				newData.label = "Suppressor: "..('<span style="color:green;">%s</span>'):format("Założone")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.SuppressorEquipped,name))
			else
				newData.label = "Suppressor: "..('<span style="color:red;">%s</span>'):format("Zdjęte")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
