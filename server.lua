...
AddEventHandler("esx_policeArmory:weaponTakenOut", function(weapon,giveAmmo)
    ...
    local id = ESX.GetPlayerFromId(source).getIdentifier()
		...
		local message = "**" ..GetPlayerName(source).. "** [" ..xPlayer.getIdentifier().. "] **|**  **wziął** " .. ESX.GetWeaponLabel(weapon) .. " z zbrojowni policyjnej **|** " ..DATE
		...
		TriggerClientEvent("esx:showNotification", source, " ~y~Wziąłeś~s~ 1x ~r~" .. ESX.GetWeaponLabel(weapon).."~r~")
    end
...
AddEventHandler("esx_policeArmory:weaponInStock", function(weapon,ammo,giveAmmo)
    ...
    if xPlayer then
		...
		local DATE = os.date("%H:%M (%d.%m.%y)")
		local message = "**" ..GetPlayerName(source).. "** [" ..xPlayer.getIdentifier().. "] **|**  **odłożył** " .. ESX.GetWeaponLabel(weapon) .. " z powrotem do Zbrojowni Policyjnej **|** " ..DATE
		PerformHttpRequest(""..Config.DiscordWebook.."", function(err, text, headers) end, 'POST', json.encode({username = "Police Armory", content = message}), { ['Content-Type'] = 'application/json' })
        TriggerClientEvent("esx:showNotification", source, " ~y~ Odłożyłeś~s~ 1x ~r~" .. ESX.GetWeaponLabel(weapon) .. "~r~")
    end
